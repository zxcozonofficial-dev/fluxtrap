...
// =====================================================================
// SLIDE 2 — AGENDA / REGLAMENT
// =====================================================================
{
  const s = p.addSlide();
  s.background = { color: ICE };
  kicker(s, "план выступления");
  s.addText("Кто о чём расскажет", {
    x: 0.7, y: 0.85, w: 10, h: 0.7,
    fontFace: FONT_HEAD, fontSize: 30, bold: true, color: NAVY_D,
    isTextBox: true, margin: 0,
  });
  s.addText("Четыре части одного рассказа: от задачи — к истории, символике и живому отношению петербуржцев к своему гимну.", {
    x: 0.7, y: 1.5, w: 11.5, h: 0.5,
    fontFace: FONT_BODY, fontSize: 13, color: GREY,
    isTextBox: true, margin: 0,
  });

  const rows = [
    ["1", "Никита", "Вступление, цель и задачи проекта", "≈ 1–1,5 мин"],
    ["2", "Тимур", "История создания гимна", "≈ 1,5–2 мин"],
    ["3", "Денис", "Символика: что музыка говорит о городе", "≈ 1,5–2 мин"],
    ["4", "Даниил", "Опрос, сравнение и заключение", "≈ 1,5–2 мин"],
  ];
  const top = 2.35, rh = 1.08;
  rows.forEach((r, i) => {
    const y = top + i * rh;
    s.addShape("roundRect", {
      x: 0.7, y, w: 11.93, h: rh - 0.18, rectRadius: 0.08,
      fill: { color: CARD }, line: { type: "none" },
      shadow: { type: "outer", color: "1B3A5C", opacity: 0.12, blur: 6, offset: 2, angle: 90 },
    });
    circleTag(s, 1.05, y + (rh - 0.18) / 2 - 0.3, 0.6, r[0], { fill: NAVY, color: "FFFFFF", fontSize: 18 });
    s.addText(r[1], {
      x: 1.95, y: y + 0.08, w: 2.6, h: 0.5,
      fontFace: FONT_HEAD, fontSize: 16, bold: true, color: NAVY_D,
      isTextBox: true, margin: 0,
    });
    s.addText(r[2], {
      x: 4.6, y: y + 0.08, w: 6.1, h: 0.6, valign: "middle",
      fontFace: FONT_BODY, fontSize: 13, color: TEXT,
      isTextBox: true, margin: 0,
    });
    s.addText(r[3], {
      x: 10.6, y: y, w: 1.85, h: rh - 0.18, valign: "middle", align: "right",
...
// =====================================================================
// SLIDE 4 — ТИМУР: история создания (таймлайн)
// =====================================================================
{
  const s = p.addSlide();
  s.background = { color: ICE };
...
// =====================================================================
// SLIDE 6 — ДАНЯ: практика (опрос + сравнение)
// =====================================================================
{
  const s = p.addSlide();
  s.background = { color: ICE };
